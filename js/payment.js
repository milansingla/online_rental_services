...
document.addEventListener("DOMContentLoaded", () => {
  ...
  onAuthStateChanged(auth, (user) => {
    ...
    document.getElementById("back-to-rent").href =
      `rent.html?id=${rentService.serviceId}`;
...
function updateOrderSummary(rentService) {
  const durationText =
    rentService.duration === "hour"
      ? "1 Hour"
      : rentService.duration === "day"
        ? "1 Day"
        : "1 Week";

  document.getElementById("order-description").textContent =
    `${rentService.serviceName} (${durationText})`;
  document.getElementById("order-price").textContent =
    `$${formatCurrency(rentService.price)}`;
