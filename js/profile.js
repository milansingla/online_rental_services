...
async function fetchUserRentals(userId) {
  ...
  try {
    ...
    rentalsContainer.innerHTML = "<p>Error loading your rentals. Please try again later.</p>"
  }
}
...
function createRentalElement(rental) {
  ...
  const statusClass = rental.status === "active" ? "status-active" : "status-expired"

  // Service image
  const serviceImage =
    rental.service.image || `https://via.placeholder.com/60x30?text=${encodeURIComponent(rental.service.name)}`
    ...
    if (e.target.closest(".view-credentials")) {
      ...
      try {
        ...
        if (rentalDoc.exists) {
          ...
          const service = serviceDoc.exists ? serviceDoc.data() : { name: "Unknown Service" }

          // Update modal content
          document.getElementById("modal-service-name").textContent = service.name
          document.getElementById("modal-service-image").src =
            service.image || `https://via.placeholder.com/120x60?text=${encodeURIComponent(service.name)}`
          document.getElementById("modal-username").textContent = rental.credentials.username
          document.getElementById("modal-password").textContent = rental.credentials.password
          document.getElementById("modal-expiry").textContent = formatDate(rental.expiryDate)
