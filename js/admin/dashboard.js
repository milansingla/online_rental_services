...
async function fetchDashboardData() {
  try {
    ...
    rentalsSnapshot.forEach((doc) => {
      ...
      if (rental.serviceId) {
        servicePopularity[rental.serviceId] =
          (servicePopularity[rental.serviceId] || 0) + 1;
      }
    });
    ...
    document.getElementById("avg-rental").textContent =
      `$${formatCurrency(avgRental)} per rental`;
